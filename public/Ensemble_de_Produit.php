...
function Panier(n) { 
    ...
    if (document.getElementById("liste").rows[n].cells[0].outerHTML.substring(5, 6) == "<") {
        var id = document.getElementById("liste").rows[n].cells[0].outerHTML.substring(4, 5);
    }
    else {
        var id = document.getElementById("liste").rows[n].cells[0].outerHTML.substring(4, 6);
    }
...
/*$nb = $connection->query("SELECT COUNT(id_produit) AS nb FROM Produit");
...
//ORDER BY id_produit ASC LIMIT (($cpage-1)*$parpage),$parpage




$rep = $connection->query("SELECT id_produit, categorie, marque, prix, date_de_peremption, reduction, quantite, image  FROM Produit")->fetchAll();
...
echo '<br/><br/><br/><br/><br/><a href="index.php">Retour</a><br>';
