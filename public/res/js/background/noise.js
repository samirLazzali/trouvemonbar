...
(function(global) {
	...
	/*
	 * for(var i=0; i<256; i++) { perm[i] = perm[i + 256] = p[i]; gradP[i] =
	...
	module.simplex2 = function(xin, yin) {
		...
		var x1 = x0 - i1 + G2; // Offsets for middle corner in (x,y) unskewed
								// coords
		var y1 = y0 - j1 + G2;
		var x2 = x0 - 1 + 2 * G2; // Offsets for last corner in (x,y) unskewed
									// coords
		var y2 = y0 - 1 + 2 * G2;
	...
	module.perlin3 = function(x, y, z) {
		...
		var n111 = gradP[X + 1 + perm[Y + 1 + perm[Z + 1]]].dot3(x - 1, y - 1,
				z - 1);
		...
		// Interpolate
		return lerp(lerp(lerp(n000, n100, u), lerp(n001, n101, u), w), lerp(
				lerp(n010, n110, u), lerp(n011, n111, u), w), v);
	};
