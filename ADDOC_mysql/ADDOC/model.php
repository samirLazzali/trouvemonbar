...
function create_company($organisation_name,$url,$email_user) {
	...
	$req1 = 'INSERT INTO company (name, url, id_user) VALUES (:name, :url, (SELECT id FROM user WHERE email=:email))';
	$val1 = array(	'name' => $organisation_name,
					'url' => $url,
					'email' => $email_user
				  );

	$req2 = 'INSERT INTO employee (id_company, id_user) VALUES ((SELECT id FROM company WHERE id_user=(SELECT id FROM user WHERE email=:email)), (SELECT id FROM user WHERE email=:email))';
	$val2 = array(	'email' => $email_user );

	db_transaction($dbb,array($req1,$req2),array($val1,$val2));
}
...
function create_professional_account($firstname,$lastname,$email,$password,$organisation_name,$url) {
	...
	$req1 = 'INSERT INTO user (firstname, lastname, email, pwd) VALUES (:firstname, :lastname, :email, :pwd)';
	$val1 = array(	'firstname' => $firstname,
					'lastname' => $lastname,
					'email' => $email,
					'pwd' => $password
				 );

	$req2 = 'INSERT INTO company (name, url, id_user) VALUES (:name, :url, (SELECT id FROM user WHERE email=:email))';
	$val2 = array(	'name' => $organisation_name,
					'url' => $url,
					'email' => $email
				  );
	...
	$val3 = array(	'email' => $email );

	/*$req4 = 'INSERT INTO directory (name, annotations, type, id_user) VALUES (:name, :annotations, :type, (SELECT id FROM user WHERE email=:email));';
	$val4 = array(	'name' => 'home',
					'type' => 'home',
					'annotations' => '',
					'email' => $email);*/
	...
	db_transaction($dbb,array($req1,$req2,$req3),array($val1,$val2,$val3));
}



function create_personnal_account($firstname,$lastname,$email,$password) {
	$req = 'INSERT INTO user (firstname, lastname, email, pwd) VALUES (:firstname, :lastname, :email, :pwd)';
	...
	/*$req2 = 'INSERT INTO directory (name, annotations, type, id_user) VALUES (:name, :annotations, :type, (SELECT id FROM user WHERE email=:email));';
	$val2 = array(	'name' => 'home',
					'type' => 'home',
					'annotations' => '',
					'email' => $email);*/
	...
	db_execute(db_prepare($dbb,$req),$val);
}
...
function update_password($old_password,$new_password) {
	...
	if($user_data[0]==-1) {
		...
		$array = array("password" => $new_password, "id" => $_SESSION['id']);

		db_execute(db_prepare($dbb,$query),$array);
...
function connect_user($email,$password) {
	...
	global $dbb;
	$query = 'SELECT id,firstname,lastname,email,date_signedup FROM user WHERE email=? AND pwd=?';
	$req = db_execute(db_prepare($dbb,$query),array($email,$password));
...
function no_user_with_email($email) {
	...
	$req = db_execute(db_prepare($dbb,$query),array($email));
...
function get_company_name() {
	...
	$query = 'SELECT name FROM company WHERE id=(SELECT id_company FROM employee WHERE id_user=:id_user)';
	$array = array("id_user" => $_SESSION['id']);
	$req = db_execute(db_prepare($dbb,$query),$array);
...
function get_aside_file($content) {
	...
	$query = 
		'SELECT id, id_user, date_creation, name FROM deposit WHERE (id_user IN (SELECT id_user FROM employee WHERE id_company=(SELECT id_company FROM employee WHERE id_user=:id_user)) OR id_user=:id_user)';
	$array = array('id_user' => $_SESSION['id']);
	$req = db_execute(db_prepare($dbb,$query),$array);
	...
	$query2 = 'SELECT id, name FROM directory WHERE id_user=?';
	$array2 = array($_SESSION['id']);
	$req2 = db_execute(db_prepare($dbb,$query2),$array2);
...
function get_deposit_structure($id) { 
	...
	$query = 'SELECT id_type FROM deposit_model WHERE id_deposit=?';
	$req = db_execute(db_prepare($dbb,$query),array($id));

	while ( ($res=db_fetch($req)) ) {
		$query2 = 'SELECT type FROM type WHERE id=?';
		$req2 = db_execute(db_prepare($dbb,$query2),array($res['id_type']));
...
function can_access_deposit($id_deposit) {
	...
	$q1 = 'SELECT id_user FROM employee WHERE id_company=(SELECT id_company FROM employee WHERE id_user=:id_user)';
	$query = 'SELECT id FROM deposit WHERE id=:id_deposit AND (id_user=:id_user OR id_user IN ('.$q1.'))';

	$array = array("id_deposit" => $id_deposit,"id_user" => $_SESSION['id']);
	$req = db_execute(db_prepare($dbb,$query),$array);
...
function can_access_directory($id_directory) {
	...
	$query0 = 'SELECT id_user FROM directory WHERE id=:id_directory AND id_user=:id_user';
	$array = array("id_directory" => $id_directory, "id_user" => $_SESSION['id']);
	$req0 = db_execute(db_prepare($dbb,$query0),$array);
	if($res0=db_fetch($req0)) return true;


	$query = 'SELECT id_deposit FROM deposit_contain WHERE id_directory=:id_directory';
	$array = array("id_directory" => $id_directory);
	$req = db_execute(db_prepare($dbb,$query),$array);
...
function get_content_deposit($id) { 
	...
	$query = 'SELECT id,date_creation,id_user FROM directory WHERE id IN (SELECT id_directory FROM deposit_contain WHERE id_deposit=?)';
	$req = db_execute(db_prepare($dbb,$query),array($id));
	$query2 = 'SELECT firstname, lastname FROM user WHERE id=?';

	while ( ($res=db_fetch($req)) ) {
		$array2 = array($res['id_user']);
		$req2 = db_execute(db_prepare($dbb,$query2),$array2);
		if ( ($res2=db_fetch($req2)) )
...
function get_content_directory($id) {
	global $dbb;
	$array = array();
	$query = 'SELECT id_file, name, date_added FROM directory_contain WHERE id_directory=?';
...
function get_employee_working_with_user($id_user) {
	...
	$req = db_execute(db_prepare($dbb,$query),array("id_user" => $id_user));
	...
	while ( ($res=db_fetch($req)) ) {
		...
		$query2 = 'SELECT firstname,lastname FROM user WHERE id=?';
		$req2 = db_execute(db_prepare($dbb,$query2),array($res['id_user']));
		if ( ($res2=db_fetch($req2)) ) {
...
function get_directory_search($search) {
	...
	if(is_professional_account()) {
		...
		$query = 'SELECT id,date_creation,id_user FROM directory WHERE id_user=?';

		$q2 = 'SELECT id_deposit FROM deposit_contain WHERE id_directory=?';
		$query2 = 'SELECT name FROM deposit WHERE id=('.$q2.')';

		$req0 = db_execute(db_prepare($dbb,$query0),array());
		while ( ($res0=db_fetch($req0) )) {

			$req = db_execute(db_prepare($dbb,$query),array($res0['id']));
			if ( ($res=db_fetch($req)) ) {

				$req2 = db_execute(db_prepare($dbb,$query2),array($res['id']));
				if ( ($res2=db_fetch($req2)) ) {
					array_push($array, array(	'id_content' => $res['id'],
...
function get_should_content_deposit($id) {
	...
	while ( ($res=db_fetch($req)) ) {
		$id_type = $res['id_type'];
		$req2 = db_execute(db_prepare($dbb,$query2),array($id_type));
		if ( ($res2=db_fetch($req2)) ) {

			$query3 = 'SELECT id FROM file WHERE id_type=? AND id_user=?';
			$req3 = db_execute(db_prepare($dbb,$query3),array($id_type,$_SESSION['id']));
...
/*function get_directory_information($id) {
	global $dbb;
	$query = 'SELECT date_creation as creation_date,name,annotations,id_user FROM directory WHERE deletion_date IS NULL AND id=?';
	$query = 'SELECT creation_date,name,annotations,id_user FROM directory WHERE deletion_date IS NULL AND id=?';
	$req = db_execute(db_prepare($dbb,$query),array($id));
	if ( ($res=db_fetch($req)) ) return $res;
	return array();
}*/
...
function get_deposit_information($id) {
	...
	$query = 'SELECT name FROM deposit WHERE id=?';
	$req = db_execute(db_prepare($dbb,$query),array($id));
	if ( ($res=db_fetch($req)) ) {
		$resu['name'] = $res['name'];
		$resu['description'] = '';
	...
	$query2 = 'SELECT count(id_type) AS `num` FROM deposit_model WHERE id_deposit=?';
	$req2 = db_execute(db_prepare($dbb,$query2),array($id));
...
function get_file_information($id_file) {
	...
	if(isset($_POST['submit']) && isset($_FILES['file']) && is_your_file($_GET['file'])) {
		...
		$query = 'UPDATE file SET address=:address WHERE id=:id';
		$array = array("address" => $folder, "id" => $id_file);
		db_execute(db_prepare($dbb,$query),$array);
	...
	$query = 'SELECT address,id_type,id_user FROM file WHERE id=?';
	$req = db_execute(db_prepare($dbb,$query),array($id_file));

	if ( ($res=db_fetch($req)) ) {
		$resu['address'] = $res['address'];

		$query3 = 'SELECT type FROM type WHERE id=?';
		$req3 = db_execute(db_prepare($dbb,$query3),array($res["id_type"]));
...
function get_file_type($id_file) {
	...
	$query = 'SELECT id_type FROM file WHERE id=?';
	$req = db_execute(db_prepare($dbb,$query),array($id_file));
...
function get_header_file($content) {
	...
	if((isset($_GET['directory']) && $_GET['directory']!="")) {
		...
		$query1 = 'SELECT id_user, name FROM directory WHERE id=:id';
		$query2 = 'SELECT firstname, lastname FROM user WHERE id=:id';
		$array1 = array('id' => $_GET['directory']);
		$req1 = db_execute(db_prepare($dbb,$query1),$array1);
		...
		if(isset($_POST['submit']) && !isset($_POST['select_type']) && check_value('name_hidden') && isset($_POST['description_hidden']) && check_value('number_files_hidden') && $_POST['number_files_hidden']!=0) {
			...
			$query = 'SELECT id_user FROM employee WHERE id_company IN (SELECT id_company FROM employee WHERE id_user=:id)';
			$query0 = 'SELECT id FROM deposit WHERE name=:name AND (id_user=:id OR id_user IN ('.$query.'))';
			$array0 = array(	'id' => $_SESSION['id'],
								'name' => $_POST['name_hidden']);
			$req = db_execute(db_prepare($dbb,$query0),$array0);
			...
			if ( !($res=db_fetch($req)) ) {
				...
				$query1 = 'INSERT INTO deposit (id_user, name) VALUES (:id_user, :name)';
				$array1 = array('id_user' => $_SESSION['id'],
								'name' => $_POST['name_hidden']);
				...
				while(($i<$n) && isset($_POST['file'.$i])) {
					...
					$query2 = 'INSERT INTO deposit_model (id_deposit, id_type) VALUES ((SELECT id FROM deposit WHERE id_user=:id_user AND name=:name), (SELECT id FROM type WHERE type=:type))';
					$array2 = array(	'id_user' => $_SESSION['id'],
										'name' => $_POST['name_hidden'],
										'type' => $_POST['file'.$i]);
				...
				db_transaction($dbb,$query,$array);
...
function get_type_file_id($id_deposit) {
	...
	$req = db_execute(db_prepare($dbb,$query),array("id_deposit" => $id_deposit));
...
function is_professional_account() {
	...
	$query = 'SELECT id_company FROM employee WHERE id_user=:id_user';
	$req = db_execute(db_prepare($dbb,$query),array("id_user" => $_SESSION['id']));
...
function is_your_file($id_file) {
	...
	$req = db_execute(db_prepare($dbb,$query),array("id_file" => $id_file));
...
function account_exist($email) {
	...
	$req = db_execute(db_prepare($dbb,$query),array("email" => $email));
...
function get_user_info($id) {
	...
	$req = db_execute(db_prepare($dbb,$query),array("id" => $id));
	if($res=db_fetch($req)) {
...
function add_employee_company($email) {
	...
	$a1= array("email" => $email);
	$q2 = 'SELECT id_company FROM employee WHERE id_user=:id_user';
	$a2 = $array = array("id_user" => $_SESSION['id']);

	$r1 = db_execute(db_prepare($dbb,$q1),$a1);
	if( ($re1=db_fetch($r1)) ) {
		$r2 = db_execute(db_prepare($dbb,$q2),$a2);
		if( ($re2=db_fetch($r2)) ) {
			$query ='INSERT INTO employee (id_user,id_company) VALUES (:id_user,:id_company)';
			$req = db_execute(db_prepare($dbb,$query),array("id_user"=>$re1['id'],"id_company"=>$re2['id_company']));
		}
	}
}
...
function create_new_directory($name,$description,$id_user,$fi,$input,$n,$deposit_id,$files_array) {
	...
	$array0 = array(	'id_user' => $id_user,
						'name'=> $name);
	$req0 = db_execute(db_prepare($dbb,$query0),$array0);
	...
	$query2 = 'INSERT INTO deposit_contain (id_deposit, id_directory, annotations)  VALUES (:id_deposit, :id_directory, :annotations)';
	$array2 = array(	'id_deposit' => $deposit_id,
	...
	for($i=0;$i<$n;$i++) {
		if(isset($input['file'.$i])) {
			...
			$query3 = 'SELECT type FROM type WHERE id=?';
			$array3 = array($file_id);
			$req3 = db_execute(db_prepare($dbb,$query3),$array3);
			if ( ($res3=db_fetch($req3)) ) $name_file = $res3['type'];

		} else {
			$query3 = 'INSERT INTO file (id_type, id_user, address)  VALUES (:id_type, :id_user, :address)';
			$array3 = array(	'id_type' => $type_file[$i],
								'id_user' => $id_user,
								'address'=> $id_user."file".$i);
			$req3 = db_execute(db_prepare($dbb,$query3),$array3);

			$query4 = 'SELECT id FROM file WHERE address=:address AND id_user=:id_user';
			$array4 = array(	'address' => $id_user."file".$i,
							'id_user' => $id_user);
			$req4 = db_execute(db_prepare($dbb,$query4),$array4);
			...
			if ( ($res4=db_fetch($req4)) ) {
				...
				$query5 = 'UPDATE file SET address=:address WHERE id=:id';
				$array5 = array(	'address'=> $folder,
									'id'=> $file_id);
				$req5 = db_execute(db_prepare($dbb,$query5),$array5);
		...
		$query6 = 'INSERT INTO directory_contain (id_directory, id_file, name)  VALUES (:id_directory, :id_file, :name)';
		$array6 = array(	'id_directory'=> $id_directory,
							'id_file' => $file_id,
							'name' => $name_file);
		$req6 = db_execute(db_prepare($dbb,$query6),$array6);
